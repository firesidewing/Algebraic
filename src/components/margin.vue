<template>
  <v-container>
    ...
    <v-row class="pt-5" v-show="Type == 3">
      ...
      <v-col cols="12">
        <v-text-field label="$ USD" v-show="Conversion.Type == 0" v-model="Conversion.USD" clearable filled></v-text-field>
        <v-text-field label="$ CAD" v-show="Conversion.Type == 1" v-model="Conversion.CAD" clearable filled></v-text-field>
      </v-col>
      <v-col cols="12">
        <v-card>
          <v-card-title class="display-1" v-show="Conversion.Type == 0">USD to CAD</v-card-title>
          <v-card-title class="display-1" v-show="Conversion.Type == 1">CAD to USD</v-card-title>
          <v-card-text class="display-2" v-show="Conversion.Type == 0">{{ CompConversion }}</v-card-text>
          <v-card-text class="display-2" v-show="Conversion.Type == 1">{{ CompConversion }}</v-card-text>
        </v-card>
      </v-col>
    </v-row>
  </v-container>
  ...
  computed: {
    CompMargin: function (){
      if(this.Margin.Price > 0){
        return (((this.Margin.Price - this.Margin.Cost) / this.Margin.Price) * 100).toFixed(2) + '%';
      } else{
        return '';
      }
    ...
    CompPrice: function (){
      if(1 - (this.Price.Margin / 100) > 0){
        return '$ ' + ((this.Price.Cost) / (1 - (this.Price.Margin / 100))).toFixed(2);
      } else{
        return '';
      }      
    },
    CompConversion: function(){
      if(this.Conversion.Type == 0){ // USD to CAD
